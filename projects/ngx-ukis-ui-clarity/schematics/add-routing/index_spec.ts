...
describe('add-routing standalone', () => {
  ...
  const appOptions: ApplicationOptions = {
    name: 'ukisapp',
    projectRoot: '',
    standalone: true, // no standalone API.
    inlineStyle: false,
  ...
  it('should add Imports', async () => {
    const testImportsRoutes = [
      'ExampleRouteComponent'
    ];
    const tree = await schematicRunner.runSchematic('add-routing', ngAddOptions, appTree);
    const appRoutes = tree.readContent('/src/app/app.routes.ts');
    testImportsRoutes.every(i => {
      expect(appRoutes).toContain(i);
    });

    const testImportsConfig = [
      'routes'
    ];
    const appConfig = tree.readContent('/src/app/app.config.ts');
    testImportsConfig.every(i => {
