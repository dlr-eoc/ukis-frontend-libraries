...
let layer1, layer2, layergroup1, layer3, layer4;
...
describe('LayersService', () => {
  ...
  it('should move group or layer up and down in the array', async(() => {
    const service: LayersService = TestBed.get(LayersService);
    /** layer2 == 0, layer1 == 1, layer3 == 2  */
    const layers = [layer2, layergroup1, layer3];

    layers.map(layer => {
      if (layer instanceof Layer) {
        service.addLayer(layer, 'Layers');
      } else if (layer instanceof LayerGroup) {
        service.addLayerGroup(layer);
      }
    });
  ...
  //setLayerGroups(this.layergroups);
  /*
  ...
  it('should set correct zIndex in a group', async(() => {
    //setLayerIndexInGroup
  }));
*/
