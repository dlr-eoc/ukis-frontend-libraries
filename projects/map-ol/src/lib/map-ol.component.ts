...
export class MapOlComponent implements OnInit, OnDestroy {
  ...
  private initMap() {
    const _ol = this.mapSvc.createMap('map');
    this.map = _ol.map; //
    this.view = _ol.view; //
    ...
    if (this.layersSvc) {
      ...
      this.layersAddRemoveOn = this.layersSvc.getOverlays().subscribe(layers => {
        const _baselayerslength = this.mapSvc.getLayers('baselayers').length;
        if (layers.length != this.mapSvc.getLayers('overlays').length) {
          ...
          if (_baselayerslength > 0) {
            for (const layer of layers) {
              const ollayer = this.mapSvc.getLayerByKey({ key: 'id', value: layer.id }, 'overlays');
              if (ollayer) {
